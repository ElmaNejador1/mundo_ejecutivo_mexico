    ...
    case 'GET':
      try {
        ...
        response = await conn.query(`SELECT *
                                     ...
                                     WHERE active = true
                                     ORDER BY id DESC
                                         ${home && home === 'true' ? 'LIMIT 4' : ''};`)

        return res.status(200).json(message("Especiales obtenidos correctamente", response.rows))
      } catch (e) {
        return res.status(500).json(message("Error, al obtener los especiales"))
      }
    case 'POST':
      try {
        ...
        response = await conn.query(query_insert(await filtrar_llaves(body, keys_filter), "specials"))

        return res.status(200).json(message("Especial registrado correctamente", response.rows[0]))
      } catch (e) {
        return res.status(500).json(message("Error, al registrar el especial"))
      }
    default:
