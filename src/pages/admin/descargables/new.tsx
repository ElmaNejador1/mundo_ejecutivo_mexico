...
const New = () => {
  ...
  useEffect(() => {
    const loadData = (id: string) => {
      ...
      axios.get(`/api/v1/downloadable/${id}`, {headers: {Authorization: `${process.env.NEXT_PUBLIC_TOKEN_WEB}`}})
        .then(res => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    axios.post("/api/v1/downloadable", {
      ...dataForm,
      cover: coverImage64,
      ...
      .then(async res => {
        ...
        await router.push("/admin/descargables");
      })
      .catch(err => {
        setIsLoading(false);
  ...
  const handleDelete = (id: string) => {
    ...
    axios.delete(`/api/v1/downloadable/${id}`, {headers: {Authorization: `${process.env.NEXT_PUBLIC_TOKEN_WEB}`}})
      .then(async res => {
        toast({
              ...
              (router.query.id && typeof router.query.id === "string")
                ? (<>
                  ...
                  <ConfirmDialog
                    ...
                    titleAlertDialog={"Eliminar Descargable"}
                    messageAlertDialog={"¿Está seguro que desea eliminar este descargable?"}
                  />
                </>)
                : (<Button
                  type={"submit"}
