...
const columns = ["id", "Titulo", "Imagen", "Link"];

export const TableSpecials = () => {
  ...
  useEffect(() => {
    axios.get("/api/v1/specials", {headers: {Authorization: `${process.env.NEXT_PUBLIC_TOKEN_WEB}`}})
      .then(res => {
  ...
  const handleClick = async (id: number) => await router.push(`/admin/especiales/edit/${id}`)
