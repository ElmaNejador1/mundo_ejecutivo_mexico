...
const columns = ["id", "Titulo", "Fecha", "Portada", "Link"];

export const TableDownloadable = () => {
  ...
  useEffect(() => {
    axios.get("/api/v1/downloadable", {headers: {Authorization: `${process.env.NEXT_PUBLIC_TOKEN_WEB}`}})
      .then(res => {
  ...
  const handleClick = async (id: number) => await router.push(`/admin/descargables/edit/${id}`)
